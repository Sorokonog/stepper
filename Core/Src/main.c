...
//uint32_t my_device_id = 10;

// Variables for motor control and feedback
float angle_rad = 0;	//current angle
uint8_t target_velocity = 0;
float target_angle = 0;
uint8_t target_effort;
uint8_t velocity = 0;

uint8_t stepper_step_denominator = 1;

// set to 1  if the last goal has been reached.
char goal_status = 0;
// set to 1  if the servo stall has been detected.
char stall_status = 0;

// Functions for motor settings adjustment
void CTRL_Reg_Set();
void TORQUE_Reg_Set();
void STATUS_Reg_Set();
